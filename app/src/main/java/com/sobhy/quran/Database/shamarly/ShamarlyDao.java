...
public interface ShamarlyDao{
    ...
    @Query("select * from shamarly_mushaf where ayah_text like '%'||:text||'%'")
    List<ShamarlyAyah> getShamarlyAyatByTextWithFormation(String text);

    @Query("select * from shamarly_mushaf where ayah_text_emlaey like :text||'%'")
    List<ShamarlyAyah> getShamarlyAyatStartWith(String text);

    @Query("select * from shamarly_mushaf where ayah_text like :text||'%'")
    List<ShamarlyAyah> getShamarlyAyatStartWithWithFormation(String text);

    @Query("select * from shamarly_mushaf where ayah_text_emlaey like '%'||:text")
    List<ShamarlyAyah> getShamarlyAyatEndWith(String text);

    @Query("select * from shamarly_mushaf where ayah_text like '%'||:text")
    List<ShamarlyAyah> getShamarlyAyatEndWithWithFormation(String text);
    ...
    @Query("select * from shamarly_mushaf where ayah_id= :ayahId")
    ShamarlyAyah getShamarlyAyahByItsId(int ayahId);
